...
import { ingestConnectUcTranscriptionEvent } from "@/modules/connectuc/ingest";

// Configure this as the target of an Activepieces flow's HTTP request
// action, triggered by ConnectUC's "New Call Transcription" event. Auth is
// a shared secret (CONNECTUC_WEBHOOK_SECRET), not a user session — see
// PUBLIC_PATHS in src/proxy.ts.
export async function POST(request: Request) {
  try {
    requireWebhookSecret(request, env.CONNECTUC_WEBHOOK_SECRET);
    const body = (await request.json()) as Record<string, unknown>;
    const result = await ingestConnectUcTranscriptionEvent(body);
